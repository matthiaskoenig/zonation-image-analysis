...
class LeveledRoi:
    ...
    def get_down_sized_level(self, level: PyramidalLevel) -> Tuple[
        np.ndarray, Tuple[int, int, int, int]]:
        if (level < 4):
            raise NotImplementedError(
                "Resizing for lower levels than four is not supported"
                "so far.")

        arr, _ = self.get_by_level(PyramidalLevel.FOUR)
        return self._resize_mask(level, arr)

    def _resize_mask(self, target_level: PyramidalLevel, arr: dask.array.core.Array) -> Tuple[
        np.ndarray, Tuple[int, int, int, int]]:
        ...
        resized_image = cv2.resize(bgr_image,
                                   (new_w, new_h),
...
class ZarrImage:
    ...
    def _initialize(self):
        self.data = zarr.open_group(self._file_manager.get_zarr_file(self.name),
                                    mode="a")

    def _initialize_original_image(self) -> Dict[int, dask.array.Array]:

        with imread(self._file_manager.get_image_path(self.name),
                    aszarr=True,
    ...
    def create_multilevel_group(self, zarr_group: ZarrGroups, roi_no: int,
    ...
    def get_liver_mask(self, roi_no: int, level: PyramidalLevel) -> np.ndarray:
        if not str(roi_no) in self.data.get(ZarrGroups.LIVER_MASK.value).keys():
            raise FileNotFoundError(f"This roi number ({roi_no}) does not"
                                    f" exist in the group '{ZarrGroups.LIVER_MASK.value}' for"
                                    f"image '{self.name}'.")

        if not str(level.value) in self.data.get(
            f"{ZarrGroups.LIVER_MASK.value}/{roi_no}").keys():
            next_higher_key, arr = self._get_next_lower_key(level,
                                                            ZarrGroups.LIVER_MASK,
                                                            roi_no)
            return self._resize_mask(
                PyramidalLevel.get_by_numeric_level(int(next_higher_key)),
                level,
                arr)
    ...
    def _get_next_lower_key(self, level: PyramidalLevel, zarr_group: ZarrGroups,
                            roi_no: int) -> Tuple[str, zarr.Array]:
        next_lower_key = None
    ...
    def _resize_mask(self, source_level: PyramidalLevel, target_level: PyramidalLevel,
        ...
        resized_image = cv2.resize(single_channel_image,
                                   (new_w, new_h),
                                   interpolation=cv2.INTER_NEAREST)
