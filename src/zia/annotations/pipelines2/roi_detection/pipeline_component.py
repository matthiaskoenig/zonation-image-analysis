...
from zia.annotations.pipelines2.roi_detection.find_rois import RoiSegmentation
...
class RoiFinderComponent(IPipelineComponent):
    ...
    def run(self, data_store: DataStore, results_path: Path) -> None:
        ...
        lobe_annotations = AnnotationParser.get_annotation_by_types(
            annotations, AnnotationType.get_lobe_annnotations()
        )
        ...
        if 1 < len(liver_annotations) != len(lobe_annotations):
            raise Exception("The number of liver annotation requires a matching number of lobe annotations")

        liver_rois = RoiSegmentation.find_rois(
            data_store, annotations, lobe_annotations
        )
