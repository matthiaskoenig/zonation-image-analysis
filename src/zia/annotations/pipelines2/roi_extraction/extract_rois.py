...
def extract_roi() -> None:
    for protein, data_store in protein_data_store_dict.items():

        if overwrite:
            rois_to_do = {roi.lobe: roi for roi in data_store.rois}
        else:
            rois_to_do = {}
            for roi in data_store.rois:
                if (file_exists(subject_result_dir, protein, roi.lobe)):
                    log.info(f"ROI image exists for protein: {protein}, ROI: {roi.lobe}")
                else:
                    rois_to_do[roi.lobe] = roi
                    continue
        ...
        roi_generator_dict: Dict[str, Dict[PyramidalLevel, TileGenerator]] = {}
        ...
        for lobe_id, generator_dict in roi_generator_dict.items():
            directory = subject_result_dir / lobe_id
