...
if __name__ == "__main__":
    ...
    label_datas = [scores_pca[kmeans_pca.labels_ == label] for label in range(ncl)]

    fig, ax = plt.subplots(dpi=300)
    ax: plt.Axes

    for label_data_set in label_datas:
        ax.scatter(label_data_set[:, 0], label_data_set[:, 2], alpha=0.4)
    ax.set_xlabel("Component 1")
    ax.set_ylabel("Component 2")
    plt.show()

    poly_clustered = [list(compress(polygons, kmeans_pca.labels_ == label)) for label in range(ncl)]
