...
class SteatosisFilter:
    ...
    def _convolute_median(self, img: np.ndarray, ksize=3) -> np.ndarray:
        median_blurr = cv2.medianBlur(img, ksize)
        return median_blurr[::2, ::2]
    def _convolute_max(self, img: np.ndarray, ksize=3) -> np.ndarray:
        kernel = np.ones((ksize, ksize), dtype=np.uint8)
        max_blurr = cv2.dilate(img.astype(np.uint8), kernel)
        smoothened = cv2.GaussianBlur(max_blurr, (ksize, ksize), 0)
        return smoothened[::2, ::2]
    ...
    def _filter_img(self, img) -> np.ndarray:
        # print(img.shape)
        img = self._convolute_max(img, ksize=5)
        img = self._adaptive_hist_norm(img, ksize=(8, 8))
        img = self._convolute_median(img, ksize=3)
        img = (img / np.percentile(img, 99)) * 255
        return img.astype(np.uint8)
