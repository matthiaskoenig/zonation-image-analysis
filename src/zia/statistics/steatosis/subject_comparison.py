...
def plot_subject_comparison(slide_stats_df: pd.DataFrame,
    ...
    species_order = [sp for sp in SlideStatsProvider.species_order if sp in species_gb.groups.keys()]
    for i, species in enumerate(species_order):
        ...
        for i, (attr, ax, log, y_label) in enumerate(zip(attributes, axes[:, i], logs, labels)):

            unit = None if units is None else units[i]
